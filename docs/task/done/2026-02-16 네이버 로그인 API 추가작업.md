...
- 로그아웃 api 를 만든다. 204를 응답한다. 접근시 로그인 되어 있지 않다면 똑같이 응답을 리턴하고 아무것도 하지 않는다.
- 만약 로그인 되어 있다면 해당 멤버 아이디로 refeshToken을 검색해서 해당 리프레쉬 토큰 캐싱을 제거한다.
- 현재 java/maskun/quietchatter/security/adaptor/AuthTokenService.java 에서 해당 작업을 하고 있으나 refresh토큰을 직접 다루고
  있으므로 [AuthMemberRedisCache.java](../../../src/main/java/maskun/quietchatter/security/adaptor/out/AuthMemberRedisCache.java)
  처럼 해당 리프레쉬토큰을 다루는 작업을 따로 컴포넌트 빈으로 분리하도록 한다.

## 에이전트 작업수행내용 : 2026-02-16 14:00

- **RefreshTokenCache 분리**: `RefreshTokenCache` 인터페이스와 `RefreshTokenRedisCache` 구현체를 생성하여 리프레시 토큰 관리 책임을
  `AuthTokenService`에서 분리.
- **AuthTokenService 리팩토링**: `AuthTokenService`가 `RefreshTokenCache`를 주입받아 사용하도록 수정.
- **AuthLogoutApi 구현**: `POST /v1/auth/logout` 엔드포인트 추가. 쿠키에서 리프레시 토큰을 추출하여 캐시에서 삭제하고, 쿠키를 만료시키는 로직 구현.
- **테스트 코드 수정**: `AuthTokenServiceTest` 및 `AuthFilterTest`에서 `RefreshTokenCache` 모킹을 반영하여 테스트 수정.

## 에이전트 작업수행내용 : 2026-02-16 14:10

- **작업 완료**: 로그아웃 기능 구현 및 리프레시 토큰 관리 구조 개선 완료.
- **테스트**: `AuthTokenServiceTest` 및 `AuthFilterTest` 통과 확인.
