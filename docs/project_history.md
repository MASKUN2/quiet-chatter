...
### 엔티티 구조 단순화 및 최적화 : 2026-01-28

도메인 모델의 복잡도를 낮추고 유지보수성을 높이기 위해 주요 엔티티(Member, Book, CustomerMessage, Reaction)에 대한 대대적인 리팩토링을 수행했습니다. 불필요한 VO(Value Object) 포장과 임베디드 타입을 제거하고, 이를 직관적인 원시 타입이나 표준 Java Time API로 대체하여 구조를 단순화했습니다. 이를 통해 JDBC Batch Update와 같은 대량 데이터 처리 시 발생하던 매핑 복잡성을 해소하고, 직관적인 코드 흐름을 확보했습니다.

### API 인프라 고도화 및 편의성 개선 : 2026-01-30

API의 진화와 하위 호환성 보장을 위해 URI 기반의 API 버저닝 전략(v1)을 도입했습니다. 운영 단계에서의 트러블슈팅을 용이하게 하기 위해 SLF4J MDC(Mapped Diagnostic Context) 필터를 적용하여, 모든 웹 요청에 고유한 추적 ID를 부여하고 로그 컨텍스트를 일원화했습니다. 또한 프론트엔드 개발 생산성을 위해 `springdoc-openapi`를 활용한 JSON 기반 API 명세서 제공 기능을 구축하고, 현재 로그인된 사용자의 상태(Guest, Member, Anonymous)를 명시적으로 조회할 수 있는 엔드포인트를 추가했습니다.

### 멀티 도메인 환경 대응 및 보안 설정 : 2026-01-31

프론트엔드와 백엔드가 서로 다른 서브도메인을 사용하는 클라우드 환경(Cloudflare Pages 등)에 대응하기 위해 쿠키 정책을 재설계했습니다. 운영(Prod)과 개발(Dev/Local) 환경의 쿠키 보안 정책(Secure, SameSite, Domain)을 분리하여 관리할 수 있도록 `application-prod.yml`을 도입했습니다. 특히, 환경 설정값을 안전하고 명확하게 관리하기 위해 `@Value` 대신 `@ConfigurationProperties`를 활용한 Type-Safe 설정을 적용하여, `AuthTokenService`의 결합도를 낮추고 설정의 응집도를 높였습니다.
