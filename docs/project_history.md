...
### 도메인 모델 단순화 및 API 체계 정립 : 2026-01-30

도메인 모델의 복잡도를 낮추기 위해 불필요한 Value Object와 임베디드 타입을 제거하고, 주요 엔티티(Book, Member 등)를 직관적인 구조로 리팩토링했습니다. 아키텍처 측면에서는 URI 기반의 API 버저닝(/v1)을 도입하여 변경에 유연하게 대응할 수 있도록 했으며, JSON 기반의 API 명세서(OpenAPI/Swagger) 자동화 파이프라인을 구축하여 프론트엔드와의 협업 효율을 높였습니다. 또한, 운영 환경의 모니터링 강화를 위해 MDC 필터를 적용하여 요청별 추적 ID를 부여하고 사용자 중요 이벤트를 로깅하는 체계를 마련했습니다.

### 보안 쿠키 및 인증 전략 고도화 : 2026-01-31

프론트엔드와 백엔드의 도메인이 분리된 환경(Cross-Subdomain)을 지원하기 위해 쿠키 설정을 전면 개편했습니다. 환경별(Dev/Prod)로 `SameSite`, `Domain`, `Secure` 속성을 차별화할 수 있는 `AppCookieProperties` 구성 요소를 도입했으며, `ResponseCookie`를 통해 브라우저 보안 정책을 준수하면서도 인증 상태를 유지할 수 있는 메커니즘을 구현했습니다. 아울러 액세스 토큰 만료 시 리프레시 토큰을 활용한 자동 승계(Fallback) 로직을 필터 레벨에서 강화하여 사용자 경험을 개선했습니다.