...
### 인증 안정화 및 운영 환경 개선 : 2026-01-31

사용자 경험 개선과 시스템 안정성을 확보하기 위해 인증 메커니즘을 고도화했습니다. Access Token 만료 시 Refresh Token을 활용하여 자동으로 새로운 토큰을 발급하고 기존 토큰을 폐기하는 토큰 회전(Rotation) 전략을 적용하여 보안성을 강화하고 끊김 없는 서비스를 제공하도록 개선했습니다. 또한 분산 환경에서의 디버깅 효율성을 높이기 위해 `MDC`를 도입하여 모든 요청에 고유 추적 ID(`X-Request-ID`)를 부여하고 로깅 시스템을 체계화했습니다. 테스트 환경의 신뢰성을 위해 테스트 전용 설정을 분리하고 TestContainers 기반의 데이터베이스 환경을 표준화하여 로컬 및 CI 환경에서의 일관된 테스트 실행을 보장했습니다.
