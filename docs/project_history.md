...
### 인증 예외 처리 및 테스트 고도화 : 2026-01-31

인증 필터(`AuthFilter`)에서 토큰 갱신 중 발생하는 예외(`AuthTokenException`, `NoSuchElementException`, `AuthMemberNotFoundException`)를 적절히 처리하여, 비로그인 상태로 자연스럽게 전환되도록 개선했습니다. 이를 통해 사용자는 500 에러 대신 게스트 권한으로 서비스를 계속 이용할 수 있게 되었습니다. 또한, 테스트 환경의 신뢰성을 높이기 위해 Redis TestContainer 설정을 추가하고, `Talk` 도메인과 전역 예외 처리기(`WebExceptionHandler`)에 대한 포괄적인 단위 테스트를 작성하여 비즈니스 로직의 검증을 강화했습니다.
