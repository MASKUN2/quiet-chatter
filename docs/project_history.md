...
### 인증 토큰 로테이션 및 리액션 배치 처리 : 2026-01-31

보안과 사용자 경험을 동시에 개선하기 위해 JWT 인증 로직을 고도화했습니다. Access Token 만료 시 클라이언트의 별도 재요청 없이 서버 내부적으로 Refresh Token을 검증하여 즉시 새로운 토큰 쌍(Access/Refresh)을 발급하고 인증을 유지하는 메커니즘을 필터 레벨에 구현했습니다. 이 과정에서 기존 Refresh Token을 폐기하고 재발급하는 Rotation 전략을 적용하여 탈취된 토큰의 재사용 위험을 차단했습니다. 또한, 톡에 대한 반응(좋아요, 응원하기) 트래픽이 몰릴 경우를 대비해 DB 부하를 최소화하는 배치 처리기를 구현했습니다. 개별 건마다 DB를 갱신하는 대신, 인메모리에서 변경분을 집계(Aggregation)한 후 JDBC Batch Update를 활용해 한 번에 반영하도록 하여 쓰기 성능을 최적화했습니다.
