...
### 인증 토큰 갱신 전략 및 테스트 보강 : 2026-01-31

Access Token 만료 시 사용자에게 재로그인을 강제하는 불편함을 해소하기 위해 인증 필터단에서 Refresh Token을 활용한 자동 갱신 메커니즘을 구현했습니다. Access Token이 없거나 만료된 경우, 유효한 Refresh Token이 존재하면 자동으로 새로운 Access Token과 Refresh Token을 발급(Rotation)하고 기존 토큰을 폐기하여 보안성을 강화했습니다. 이 과정은 클라이언트에게 투명하게 처리되도록 설계되었습니다. 또한 비즈니스 로직의 안정성을 보장하기 위해 도메인 엔티티와 서비스 계층에 대한 단위 테스트를 보강하여 불변식 검증과 예외 처리 로직의 신뢰성을 확보했습니다.
