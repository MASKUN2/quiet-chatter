...
### 인증 예외 처리 개선 및 테스트 환경 격리 : 2026-01-31

사용자 경험을 저해하는 잦은 로그아웃 문제를 해결하기 위해 인증 필터 단계에서 토큰 갱신 로직을 통합했습니다. 액세스 토큰이 유효하지 않은 경우 즉시 실패 응답을 보내는 대신, 리프레시 토큰을 확인하여 자동으로 토큰을 재발급하고 요청을 이어가도록 개선했습니다. 이를 통해 클라이언트의 재시도 부담을 줄이고 서비스의 연결성을 강화했습니다. 또한, 운영 환경과 테스트 환경의 설정이 혼재되어 발생할 수 있는 부작용을 방지하기 위해 테스트 전용 설정을 분리하고, 주요 비즈니스 로직에 대한 단위 테스트를 보강하여 안정적인 배포 기반을 마련했습니다.
