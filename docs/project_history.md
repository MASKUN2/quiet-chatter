...
### 액세스 토큰 만료 시 자동 갱신 및 폴백 처리 : 2026-01-31

사용자 경험 향상과 보안 강화를 위해 JWT 인증 필터의 예외 처리 로직을 개선했습니다. 액세스 토큰이 만료되거나 누락된 경우, 유효한 리프레시 토큰이 존재하면 즉시 새로운 액세스 토큰과 리프레시 토큰을 재발급하는 'Refresh Token Rotation' 전략을 적용했습니다. 이를 통해 사용자는 토큰 만료 시 별도의 재로그인 과정 없이 서비스를 지속적으로 이용할 수 있으며, 사용된 리프레시 토큰은 즉시 폐기되어 토큰 탈취 시의 보안 위험을 최소화했습니다.
