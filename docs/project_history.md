...
### 사용자 행동 추적 및 모니터링 : 2026-01-29

운영 환경에서의 문제 해결과 사용자 행동 분석을 위해 관측성(Observability)을 강화했습니다. `MDC(Mapped Diagnostic Context)`를 도입하여 모든 HTTP 요청에 고유한 추적 ID(Trace ID)를 부여함으로써, 분산 환경에서도 로그를 요청 단위로 추적할 수 있는 기반을 마련했습니다. 또한, 주요 비즈니스 로직 수행 시 사용자 이벤트를 로깅하도록 설계하여 추후 데이터 분석을 위한 초석을 다졌습니다. 도서 검색 기능에는 무한 스크롤을 위한 페이징 기능을 추가하여 사용자 경험을 개선했습니다.

### API 명세 자동화 및 표준화 : 2026-01-30

프론트엔드와의 원활한 협업과 AI 에이전트의 API 이해도를 높이기 위해 API 명세 방식을 표준화했습니다. 기존의 정적 문서 관리 방식의 한계를 극복하고자 테스트 코드 기반의 문서화 도구인 `Spring Rest Docs`와 `restdocs-api-spec`을 결합했습니다. 이를 통해 테스트가 통과된 신뢰할 수 있는 API 명세를 OpenAPI(Swagger) 호환 JSON 포맷으로 자동 생성하고, `/api/v1/spec` 엔드포인트를 통해 실시간으로 제공하도록 구현했습니다. 또한 CI/CD 파이프라인 최적화를 위해 문서 생성용 테스트만 선별적으로 수행하는 태스크를 구성하여 빌드 속도를 확보했습니다.

### 인증 및 쿠키 정책 고도화 : 2026-01-31

프론트엔드(Cloudflare Pages)와 백엔드(API 서버)가 서로 다른 서브도메인을 사용하는 환경(`quiet-chatter.com`, `api.quiet-chatter.com`)에 맞춰 쿠키 정책을 재설계했습니다. 운영 환경(`prod`)과 개발 환경의 설정을 분리하여, 운영 환경에서는 `Domain` 설정과 `SameSite`, `Secure` 속성을 엄격하게 적용하여 보안을 강화하고 도메인 간 쿠키 공유가 가능하도록 했습니다. 이 과정에서 설정의 안정성을 위해 `Type-safe Configuration` 패턴을 도입하여 쿠키 설정을 객체로 관리했습니다. 또한, 액세스 토큰 만료 시 클라이언트의 별도 재요청 없이도 리프레시 토큰을 통해 투명하게 인증이 유지되도록 서버 측 Fallback 로직을 구현하여 끊김 없는 사용자 경험을 제공했습니다.
