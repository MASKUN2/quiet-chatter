...
### 인증 예외 처리 강화 및 테스트 환경 격리 : 2026-01-31

인증 과정에서 발생하는 예외 상황을 보다 유연하게 처리하도록 `AuthFilter`를 개선했습니다. 액세스 토큰 만료 시(`ExpiredAuthTokenException`) 즉시 실패 처리하는 대신 리프레시 토큰을 통한 재발급을 시도하여 사용자 경험을 끊김 없이 유지하도록 했으며, 기타 인증 관련 예외(`AuthTokenException`) 발생 시에는 `null`을 반환하여 익명 사용자로 처리되도록 변경했습니다. 또한 테스트 환경의 독립성을 보장하기 위해 `src/test/resources/application.yml`을 신설하여 운영 환경 설정과의 의존성을 제거했습니다. 이와 함께 `Talk` 도메인의 생성 및 수정 로직, 그리고 전역 예외 처리기(`WebExceptionHandler`)에 대한 단위 테스트를 보강하여 비즈니스 로직의 안정성을 확보했습니다.
