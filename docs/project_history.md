...
### 테스트 커버리지 확대 및 보안 설정 검증 : 2026-01-31

서비스 안정성을 강화하기 위해 핵심 비즈니스 로직과 보안 설정에 대한 테스트 전략을 보강했습니다. `Talk` 도메인의 불변식과 상태 변경 로직을 검증하는 단위 테스트를 작성하여 도메인 무결성을 확보하고, `MockMvc`를 활용한 웹 계층 테스트를 통해 예외 처리의 일관성을 검증했습니다. 특히, 잘못된 설정 시 보안 사고로 이어질 수 있는 CORS 정책에 대해 허용/거부 시나리오별 검증 테스트를 추가하여 신뢰도를 높였습니다. 아울러 테스트 환경의 독립성을 보장하기 위해 테스트 전용 설정 파일을 분리하여 운영 환경 설정과의 간섭을 제거했습니다.